...
export function MemoryPacks() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      if (exportFormat === 'md') {
        downloadMarkdown(packToMarkdown(pack), `${slug}.cogxai-pack.md`);
      } else {
        const blob = new Blob([JSON.stringify(pack, null, 2)], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `${slug}.cogxai-pack.json`;
        a.click();
        ...
        <div style={{ border: '1px solid var(--border)' }}>
          ...
          <div style={{ padding: 20 }}>
            ...
            <div style={{ marginTop: 32, padding: 16, background: 'var(--bg-warm)', fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.7 }}>
              ...
              <div>On-chain memory marketplace. License memory packs for $COGXAI. Smart contract verification for pack authenticity.</div>
            </div>
          </div>
        </div>
      </div>
