...
export function useAuth(): AuthState {
  ...
  // Extract wallet address from connected wallets (prefer EVM/ethereum)
  const walletAddress = useMemo(() => {
    if (!wallets || wallets.length === 0) return null;
    const evmWallet = wallets.find(w => w.walletClientType === 'metamask' || w.walletClientType === 'coinbase_wallet' || (w as any).chainType === 'ethereum');
