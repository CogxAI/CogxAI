...
try {
  ...
  if (env.includes('CORTEX_API_KEY=') && !env.includes('CORTEX_API_KEY=your-api-key')) {
    ...
    writeTty(`  ${gray}Run ${reset}${cyan}npx cogxai setup${reset}${gray} to reconfigure.${reset}\n\n`);
    writeTty(`${dim}────────────────────────────────────────────────────${reset}\n`);
    process.exit(0);
  }
} catch {}
...
if (!canReadTty) {
  writeTty(`\n  ${bold}To set up, run:${reset}  ${cyan}npx cogxai setup${reset}\n`);
  writeTty(`  ${gray}Or visit:${reset}       ${cyan}https://cogxai.org/setup${reset}\n\n`);
  writeTty(`${dim}────────────────────────────────────────────────────${reset}\n`);
  process.exit(0);
}
...
step(1, 3, 'Register');
info('We\'ll create an account on cogxai.org and get you an API key.');
...
if (existingKey) {
  ...
  writeTty('\n');
  writeTty(`  ${gray}Registering on cogxai.org...${reset}`);

  try {
    const payload = JSON.stringify({ name: agentName, wallet: wallet || 'pending' });
    const result = execSync(
      `curl -s -X POST https://cogxai.org/api/cortex/register -H "Content-Type: application/json" -d '${payload.replace(/'/g, "'\\''")}'`,
    ...
    if (data.apiKey) {
      ...
      info('Run npx cogxai register later to get a key');
    }
  } catch (err) {
    writeTty('\r' + ' '.repeat(50) + '\r');
    ...
    if (/certificate|CERT|SSL|self.signed|unable to verify/i.test(msg)) {
      ...
      info('Try a mobile hotspot/VPN, or ask IT to whitelist cogxai.org');
    } else {
      warn('Could not reach cogxai.org');
    }
    info('Run npx cogxai register later to get a key');
  }
}
...
function installMcp(configPath) {
  const entry = {
    ...
    env: {
      ...
      CORTEX_HOST_URL: 'https://cogxai.org',
      ...(wallet ? { COGXAI_WALLET: wallet } : {}),
      ...(agentName ? { COGXAI_AGENT_NAME: agentName } : {}),
    },
  };
  try {
    let config = {};
    ...
    config.mcpServers['cogxai-memory'] = entry;
...
if (mcpChoice && mcpChoice !== '5') {
  ...
  if (toInstall.length === 0) {
    info('Skipped — run npx cogxai mcp-install anytime');
  } else {
    for (const t of toInstall) {
      if (installMcp(t.path)) {
        ok(`Added cogxai-memory to ${t.label}`);
...
else warn('No API key yet — run: npx cogxai register');
ok('.env created');

writeTty(`\n  ${bold}Next steps:${reset}\n`);
writeTty(`  ${dim}Explorer:${reset}   ${cyan}https://cogxai.org/explore${reset}\n`);
writeTty(`  ${dim}Docs:${reset}       ${cyan}https://cogxai.org/docs${reset}\n`);
writeTty(`  ${dim}Reconfigure:${reset} ${cyan}npx cogxai setup${reset}\n`);
writeTty(`${dim}────────────────────────────────────────────────────${reset}\n\n`);
