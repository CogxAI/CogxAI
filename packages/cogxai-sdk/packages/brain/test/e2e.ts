/**
 * End-to-end test for @cogxai/brain
 *
 * Tests the full pipeline: store → embed → recall → score → link → decay
 */
import { CogxAIEngine } from '../src/engine.js';
...
async function main() {
  ...
  assert(m1.id.startsWith('cogxai-'), 'Memory has hash ID');
  ...
  const scopedEngine = new CogxAIEngine({
    storage,
    embeddings,
