...
import { cogxaiTools, handleCogxAITool } from '../src/adapters/openai.js';
import { CogxAIMemory } from '../src/adapters/langchain.js';
...
async function main() {
  ...
  assert(pack.format === 'cogxai-pack', 'Pack format');
  ...
  assert(cogxaiTools.length === 3, `${cogxaiTools.length} tools defined`);
  assert(cogxaiTools[0].function.name === 'cogxai_remember', 'remember tool');
  assert(cogxaiTools[1].function.name === 'cogxai_recall', 'recall tool');
  assert(cogxaiTools[2].function.name === 'cogxai_forget', 'forget tool');

  // Mock engine
  const storedMemories: any[] = [];
  const mockEngine = {
    store: async (opts: any) => {
      const mem = { id: 'cogxai-test123', memory_type: opts.type || 'episodic', ...opts };
  ...
  // Test remember
  const rememberResult = await handleCogxAITool(
    { function: { name: 'cogxai_remember', arguments: JSON.stringify({ content: 'User likes dark mode', type: 'semantic' }) } },
    mockEngine,
  );
  ...
  // Test recall
  const recallResult = await handleCogxAITool(
    { function: { name: 'cogxai_recall', arguments: JSON.stringify({ query: 'user preferences' }) } },
    mockEngine,
  );
  ...
  // Test forget
  const forgetResult = await handleCogxAITool(
    { function: { name: 'cogxai_forget', arguments: JSON.stringify({ id: 'cogxai-test123' }) } },
    mockEngine,
  );
  ...
  const lcMemory = new CogxAIMemory(mockEngine);
