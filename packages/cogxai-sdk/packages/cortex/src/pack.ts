...
export interface MemoryPack {
  ...
  meta: {
    ...
    entity_count: number;
    content_hash: string;
    /** HMAC-SHA256 (local) or Base secp256k1 signature */
    signature?: string;
    signature_type?: 'hmac-sha256' | 'secp256k1';
    /** Base transaction ID if committed on-chain */
    base_tx?: string;
  };
}
 ...
 * For production Base signing, use @cogxai/cortex/base.
 */
...
export function packToMarkdown(pack: MemoryPack): string {
  ...
  if (pack.meta.base_tx) md += `> On-chain: [${pack.meta.base_tx.slice(0, 16)}...](https://basescan.org/tx/${pack.meta.base_tx})\n`;
  md += '\n';
...
export function packFromJSON(json: string): MemoryPack {
  ...
  if (pack.version !== 1 || pack.format !== 'cogxai-pack') {
