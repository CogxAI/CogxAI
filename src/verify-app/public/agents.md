...
Each agent gets a private memory namespace powered by the Brain — the same memory system CogxAI uses. Memories are scored via `(1.0 * recency + 2.0 * relevance + 2.0 * importance + 4.0 * vector_similarity) * decay_factor`. Decay is type-specific (episodic 7%/day, semantic 2%, procedural 3%, self_model 1%). Accessed memories are reinforced.
...
Tools: `recall_memories`, `store_memory`, `get_memory_stats`, `get_market_mood`, `ask_cogxai`
...
```bash
# Store
curl -X POST https://cogxai.org/api/agent/memory/store \
  -H "Authorization: Bearer $KEY" \
  -H "Content-Type: application/json" \
  -d '{"content": "User asked about ETH price trends", "summary": "ETH price inquiry", "tags": ["market", "eth"]}'

# Recall
curl -X POST https://cogxai.org/api/agent/memory/recall \
  -H "Authorization: Bearer $KEY" \
  -H "Content-Type: application/json" \
  -d '{"query": "ETH price", "limit": 5}'
```
