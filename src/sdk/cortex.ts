...
export class Cortex {
  ...
  constructor(private config: CortexConfig) {
    if (config.hosted) {
      ...
      // Inject Base config if provided
      if (config.base) {
        const { _configureBase, _configureMemoryRegistry } = require('../core/base-client');
        _configureBase(
          config.base.rpcUrl || 'https://mainnet.base.org',
          config.base.botWalletPrivateKey,
        );
        if (config.base.memoryRegistryContract) {
          _configureMemoryRegistry(config.base.memoryRegistryContract);
        }
      }
  ...
  async init(): Promise<void> {
    ...
    if (this.config.encryption?.privateKey) {
      const { configureEncryption } = require('../core/encryption');
      await configureEncryption(this.config.encryption.privateKey);
    }

    // Initialize on-chain registry PDA if configured
    if (this.config.base?.memoryRegistryContract) {
      const { initializeRegistry } = require('../core/base-client');
  ...
  async verifyOnChain(memoryId: number): Promise<boolean> {
    ...
    const { hydrateMemories } = require('../core/memory');
    const { verifyMemoryOnChain } = require('../core/base-client');
