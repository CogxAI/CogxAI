...
export async function runRegister(): Promise<void> {
  ...
  console.log(`  ${c.gray}This gives you an API key to store memories on COGXAI infrastructure.${c.reset}\n`);
  ...
  const wallet = await ask(rl, 'Your Base wallet address: ');
  if (!wallet || !/^0x[0-9a-fA-F]{40}$/.test(wallet)) {
    printWarn('Invalid Base address');
    rl.close();
    return;
  }

  const url = 'https://cogxai.org';
  ...
  try {
    ...
    if (isCert) {
      ...
      printInfo('Try: switch to a mobile hotspot or VPN, or ask IT to whitelist cogxai.org');
    } else {
