...
export async function runExport(): Promise<void> {
  ...
  if (hasFlag(args, '--help') || hasFlag(args, '-h')) {
    console.log(`
  ${c.bold}Usage:${c.reset}  npx cogxai export [options]
  ...
  ${c.bold}Examples:${c.reset}
    ${c.cyan}npx cogxai export${c.reset}                          Export all as JSON
    ${c.cyan}npx cogxai export --format md${c.reset}              Export as Markdown
    ${c.cyan}npx cogxai export --types episodic,semantic${c.reset} Filter by type
    ${c.cyan}npx cogxai export --output backup.json${c.reset}     Custom filename
`);
    return;
  }
  ...
  if (!isHosted && !isSelfHosted) {
    ...
    console.log(`\n  Run ${c.cyan}npx cogxai init${c.reset} to set up hosted or self-hosted mode.\n`);
    process.exit(1);
  }
  ...
  if (isHosted) {
    ...
    const baseUrl = config.cortex.hostUrl || 'https://cogxai.org';
    try {
      const res = await fetch(`${baseUrl}/api/cortex/packs/export`, {
        ...
        body: JSON.stringify({
          name: 'CLI Export',
          description: 'Exported via npx cogxai export',
          limit: 10000,
          types,
        }),
      });
  ...
  if (format === 'md') {
    defaultName = 'cogxai-memories.md';
    ...
    defaultName = 'cogxai-memories.json';
    const pack = {
      version: 1,
  ...
  console.log(`\n  ${c.dim}Drop this file into ${c.reset}${c.cyan}cogxai.org/explore${c.reset}${c.dim} to visualize.${c.reset}`);
  printDivider();
  console.log('');
}
