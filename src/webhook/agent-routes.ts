...
export function agentRoutes(): Router {
  ...
  // POST /query — ask CogxAI anything
  ...
  router.post('/roast-wallet', async (req: Request, res: Response) => {
    try {
      ...
      const response = await buildAndGenerate({
        message: `Roast this Base wallet: ${wallet}`,
        agentModifier: getAgentTierModifier(agent.tier),
        instruction:
          `Agent "${agent.agent_name}" wants you to roast a Base wallet. ` +
  ...
  // GET /memory-stats — CogxAI's memory statistics (no Claude call)
