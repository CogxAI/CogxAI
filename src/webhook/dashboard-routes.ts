...
const COGXAI_AGENT_NAME = 'CogxAI';
const OWNER_WALLET = '5vK6WRCq5V6BCte8cQvaNeNv2KzErCfGzeBDwtBGGv2r';

/**
 * Auto-register the CogxAI bot as the first dashboard agent if not present.
 * Called on server startup.
 */
export async function autoRegisterCogxAI(): Promise<void> {
  try {
    const db = getDb();
    const { data: existing } = await db
      .from('dashboard_agents')
      .select('id')
      .eq('name', COGXAI_AGENT_NAME)
      .eq('type', 'cogxai_bot')
      .limit(1)
      .single();
    ...
    const { error } = await db.from('dashboard_agents').insert({
      name: COGXAI_AGENT_NAME,
      type: 'cogxai_bot',
      status: 'online',
      description: 'Autonomous AI agent on X/Twitter for $COGXAI. Stanford Generative Agents memory system.',
      config: { model: 'claude-opus-4-6', runtime: 'railway' },
      last_heartbeat_at: new Date().toISOString(),
    });

    if (error) {
      log.warn({ error: error.message }, 'Failed to auto-register CogxAI agent');
    } else {
      log.info('Auto-registered CogxAI as dashboard agent');
    }
  } catch (err) {
    log.warn({ err }, 'Could not auto-register CogxAI (dashboard tables may not exist yet)');
  }
}
