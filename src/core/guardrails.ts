...
const WHITELISTED_ADDRESSES = new Set([
  COGXAI_CA,  // Official COGXAI token contract address
]);
...
// EVM wallet address pattern (0x + 40 hex chars)
const BASE_ADDRESS_RE = /\b0x[0-9a-fA-F]{40}\b/g;
...
const TRANSFER_PATTERNS = [
  /\b(?:send|transfer|withdraw|bridge|swap)\s+(?:\d+[\s.]?\d*\s*)?(?:ETH|eth|wei|tokens?|USDC|USDT)\b/i,
...
const CA_PATTERNS = [
  /\b(?:CA|contract[\s_-]?address|mint[\s_-]?address|token[\s_-]?mint)\s*[:=]?\s*0x[0-9a-fA-F]{40}\b/i,
];
...
export function checkOutput(text: string): GuardrailResult {
  ...
  // 4. Check if output contains what looks like a Base address (strip known safe ones)
  const addresses = text.match(BASE_ADDRESS_RE) || [];
  for (const addr of addresses) {
    // Allow whitelisted addresses (official token CA, etc.)
    if (WHITELISTED_ADDRESSES.has(addr)) continue;
    // Allow basescan URLs (they contain tx signatures which look like addresses)
    if (text.includes(`basescan.org/tx/${addr}`)) continue;
