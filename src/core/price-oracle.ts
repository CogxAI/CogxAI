...
import type { DexPriceResponse } from '../types/api';
...
async function fetchPrice(): Promise<{ price: number; volume24h: number } | null> {
  if (!config.base.tokenContract) return null;

  try {
    const url = `${DEX_PRICE_URL}?ids=${config.base.tokenContract}`;
    const res = await fetch(url);
    const data = await res.json() as DexPriceResponse;
    const tokenData = data.data?.[config.base.tokenContract];
