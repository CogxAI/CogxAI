...
function getRwClient(): TwitterApi {
  if (!_rwClient) getClient();
  return _rwClient!;
}
...
export async function postReply(tweetId: string, text: string): Promise<string> {
  ...
  const result = await getRwClient().v2.reply(truncated, tweetId);
...
export async function postTweet(text: string): Promise<string> {
  ...
  const result = await getRwClient().v2.tweet(truncated);
  return result.data.id;
}
...
export async function postThread(texts: string[]): Promise<string[]> {
  ...
  for (const text of texts) {
    ...
    if (!previousId) {
      ...
      const result = await getRwClient().v2.reply(truncated, previousId);
      previousId = result.data.id;
    }
...
/**
 * Search tweets mentioning @CogxAI with #CogxAI.
...
export async function searchHashtagTweets(
  ...
  try {
    // Broad match: direct mentions, hashtag, or $COGXAI with campaign keywords
    const query = '(@CogxAI OR #CogxAI OR ($COGXAI ("AI memory" OR "blockchain brain"))) -is:retweet';
    const params: Record<string, string> = {
...
/**
 * Search tweets mentioning $COGXAI or @CogxAI.
...
export async function searchTokenMentions(
  ...
  log.info({ sinceId, maxResults }, 'Searching $COGXAI mentions');

  try {
    const query = '($COGXAI OR @CogxAI) -is:retweet';
    const params: Record<string, string> = {
      'tweet.fields': 'created_at,author_id,public_metrics',
      'expansions': 'author_id',
      'user.fields': 'username',
