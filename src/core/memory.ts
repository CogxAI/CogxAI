...
import { writeMemo, isRegistryEnabled, registerMemoryOnChain } from './base-client';
...
// ============================================================
// THE CORTEX — CogxAI's Memory System
//
// Inspired by:
...
//   self_model  — CogxAI's evolving understanding of itself
// ============================================================
...
export interface Memory {
  id: number;
  hash_id: string;              // Collision-resistant ID like "cogxai-a1b2c3d4"
...
export async function storeMemory(opts: StoreMemoryOptions): Promise<number | null> {
  ...
  try {
    ...
    // Commit memory to Base (fire-and-forget)
...
async function commitMemoryToChain(memoryId: number, opts: StoreMemoryOptions): Promise<void> {
  // Skip mainnet commits for demo memories (they use testnet instead)
  ...
  await db
    .from('memories')
    .update({ base_signature: signature })
    .eq('id', memoryId);
