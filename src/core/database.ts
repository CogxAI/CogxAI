  ...
  try {
    const { error } = await db.rpc('exec_sql', {
      query: `
        ...
        ALTER TABLE memories ADD COLUMN IF NOT EXISTS base_signature TEXT;
        ...
        UPDATE memories
        SET hash_id = 'cogxai-' || SUBSTRING(md5(id::text || created_at::text), 1, 8)
        ...
        CREATE TABLE IF NOT EXISTS dashboard_agents (
          ...
          name TEXT NOT NULL,
          type TEXT DEFAULT 'claude_code' CHECK (type IN ('claude_code', 'script', 'webhook', 'cogxai_bot', 'content', 'research', 'dev', 'testing', 'design_audit', 'customer_journey')),
