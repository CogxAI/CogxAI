...
/**
 * MCP Server — expose CogxAI's memory and capabilities as tools.
 *
 * Supports three modes:
 * - Local: fully offline, SQLite + GTE-Small embeddings, no API keys needed
 * - Hosted: uses CORTEX_API_KEY to call the CogxAI HTTP API (no Supabase needed)
 * - Self-hosted: uses direct imports from core/memory (requires Supabase + full env)
 *
 * Run with: npx cogxai mcp-serve
 */

// ── Mode Detection ───────────────────────────────────────────────────

const CORTEX_API_KEY = process.env.CORTEX_API_KEY || '';
const CORTEX_HOST_URL = process.env.CORTEX_HOST_URL || 'https://cogxai.org';
const isLocalMode = process.argv.includes('--local') || process.env.COGXAI_LOCAL === 'true';
...
function loadSelfHosted() {
  if (!_recallMemories) {
    try {
      ...
      console.error('[cogxai-mcp] Failed to load memory module:', err);
...
server.tool(
  ...
  'Search the memory system. Returns scored memories ranked by relevance, importance, recency, and decay.',
  {
    ...
    related_wallet: z.string().optional().describe('Filter by associated Base wallet address'),
  ...
  async (args) => {
    try {
      ...
      console.error('[cogxai-mcp] recall_memories error:', err.message);
      return {
...
server.tool(
  ...
  'Store a new memory. Memories persist across conversations and decay over time if not accessed.',
  {
    ...
    related_wallet: z.string().optional().describe('Associated Base wallet address'),
  ...
  async (args) => {
    try {
      ...
      console.error('[cogxai-mcp] store_memory error:', err.message);
      return {
        content: [{ type: 'text' as const, text: JSON.stringify({ error: err.message }) }],
        isError: true,
      };
...
server.tool(
  ...
  async () => {
    try {
      ...
      console.error('[cogxai-mcp] get_memory_stats error:', err.message);
      return {
        content: [{ type: 'text' as const, text: JSON.stringify({ error: err.message }) }],
        isError: true,
      };
...
server.tool(
  ...
  async (args) => {
    try {
      ...
      console.error('[cogxai-mcp] find_clinamen error:', err.message);
      return {
        content: [{ type: 'text' as const, text: JSON.stringify({ error: err.message }) }],
        isError: true,
      };
...
async function main() {
  ...
  if (isLocalMode) {
    console.error('[cogxai-mcp] Local mode — memories stored in ~/.cogxai/memories.json');
    console.error('[cogxai-mcp] No API keys required. Fully offline.');
  } else if (isHostedMode) {
    if (CORTEX_API_KEY.length < 10) {
      console.error('[cogxai-mcp] Warning: CORTEX_API_KEY looks invalid (too short)');
    }
    console.error(`[cogxai-mcp] Hosted mode — API: ${CORTEX_HOST_URL}`);
  } else {
    const hasSupabase = !!(process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_KEY);
    if (!hasSupabase) {
      console.error('[cogxai-mcp] Warning: No CORTEX_API_KEY or Supabase config found. Tools will fail.');
      console.error('[cogxai-mcp] Set CORTEX_API_KEY for hosted mode, or SUPABASE_URL + SUPABASE_SERVICE_KEY for self-hosted.');
    }
    console.error('[cogxai-mcp] Self-hosted mode — direct database access');
  }

  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error(`[cogxai-mcp] Server started on stdio`);
}

main().catch((err) => {
  console.error('[cogxai-mcp] Fatal error:', err);
  process.exit(1);
});
