 ...
 * Zero dependencies. No API keys. Stores memories in ~/.cogxai/memories.json
 */
...
const COGXAI_DIR = join(process.env.HOME || process.env.USERPROFILE || '.', '.cogxai');
const MEMORIES_FILE = join(COGXAI_DIR, 'memories.json');
...
function ensureDir(): void {
  if (!existsSync(COGXAI_DIR)) {
    mkdirSync(COGXAI_DIR, { recursive: true });
  }
}
