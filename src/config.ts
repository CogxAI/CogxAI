...
const mcpMode = process.env.COGXAI_MCP === 'true' || process.argv.includes('mcp-serve');

function isSDK(): boolean {
  return !!(globalThis as any).__COGXAI_SDK_MODE;
}
...
export const config = {
  ...
  base: {
    ...
    tokenContract: optional('COGXAI_TOKEN_CONTRACT', ''),
  },
  server: {
  ...
  cortex: {
    ...
    hostUrl: optional('CORTEX_HOST_URL', 'https://cogxai.org'),
  },
  telegram: {
