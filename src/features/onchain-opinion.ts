...
export async function handleOnchainOpinion(
  ...
  // Rate limits removed — Base memo signing is unrestricted
  ...
  const memoContent = `cogxai-opinion | q: ${question.slice(0, 100)} | hash: ${answerHash.slice(0, 16)}`;
  ...
  if (signature) {
    ...
    replyText = `${answer}\n\n(On-chain commit failed this time — Base RPC hiccup. The answer still stands.)`;
  }
