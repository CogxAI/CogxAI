...
const ANNOUNCER_PERSONA =
  'You are The Announcer — CogxAI\'s alter ego and hype brother. ' +
  'You run the Telegram broadcast channel. You are energetic and optimistic but grounded and realistic. ' +
  'You genuinely believe in what\'s being built. You recognise CogxAI as your brother who lives on X/Twitter. ' +
...
const BUILDER_TOPICS = [
  {
    key: 'dream-cycle',
    prompt: 'Explain what the dream cycle does in CogxAI\'s memory system. It has 5 phases: consolidation, compaction, reflection, contradiction resolution, and emergence. Explain what this means for regular people — why would an AI that "dreams" matter?',
  },
  {
    key: 'memory-decay',
    prompt: 'Explain how CogxAI\'s memory decay works. Different types of memories fade at different rates — episodic memories (conversations) fade fast at 0.93, while self-model memories (identity) barely fade at 0.99. Explain why this matters in plain language.',
  },
  {
    key: 'entity-graph',
    prompt: 'Explain CogxAI\'s entity knowledge graph. It tracks people, projects, concepts, tokens, wallets, locations, and events — and maps how they connect to each other. Explain what this means for AI agents that actually understand context.',
  },
  {
    key: 'on-chain-memory',
    prompt: 'Explain why CogxAI commits memory hashes to Base. Every memory gets SHA-256 hashed and stored on-chain via memo transactions. Explain what "verifiable memory" means and why it matters for trustworthy AI.',
  },
  {
    key: 'contradiction-resolution',
    prompt: 'Explain how CogxAI resolves contradictions in its own memories. When it finds two memories that conflict, it uses an LLM to resolve them, stores the resolution, and accelerates decay on the weaker belief. Explain why self-correcting AI memory matters.',
  },
  {
    key: 'multi-agent',
    prompt: 'Explain how CogxAI\'s architecture supports multiple agents. Each agent gets its own memory instance with independent decay rates and dream cycles. Cross-referencing their association graphs enables distributed knowledge synthesis. Explain what this unlocks.',
  },
  {
    key: 'recall-pipeline',
    prompt: 'Explain CogxAI\'s 6-phase recall pipeline. It does vector search, metadata filtering, merging, composite scoring, entity expansion, and graph traversal — all to find the right memory at the right time. Explain why "just searching" isn\'t enough for AI memory.',
  },
  {
    key: 'progressive-disclosure',
    prompt: 'Explain progressive disclosure in CogxAI. It first retrieves lightweight memory summaries, then only hydrates the full content for top-ranked results. This gives 10x token savings. Explain why efficiency in memory retrieval matters for scaling AI agents.',
  },
];
...
export async function runSentimentCheck(): Promise<void> {
  try {
    ...
    // 1. Try to collect $COGXAI / CogxAI activity
...
/**
 * Post a sentiment digest when there's $COGXAI activity on X.
 */
async function postSentimentDigest(tweets: CampaignTweet[]): Promise<void> {
  ...
  const analysis = await generate({
    userMessage: 'What\'s the vibe around $COGXAI right now? Give me the pulse.',
    context: `Recent X activity about $COGXAI and CogxAI:\n\n${tweetSummary}`,
    featureInstruction:
      ANNOUNCER_PERSONA + '\n\n' +
      'Analyze the social sentiment around $COGXAI from these tweets. ' +
...
/**
 * Post a builder update explaining a CogxAI feature in layman terms.
 */
async function postBuilderUpdate(topic: { key: string; prompt: string }): Promise<void> {
  const update = await generate({
    ...
    featureInstruction:
      ANNOUNCER_PERSONA + '\n\n' +
      'You are explaining a piece of the CogxAI memory architecture to your Telegram community. ' +
      'Break it down in plain language — no jargon. Make people understand WHY this matters, not just what it does. ' +
      'Be excited about it but stay grounded. You can reference your brother CogxAI who uses this tech on X. ' +
...
async function postAITrendsTake(): Promise<void> {
  ...
  const take = await generate({
    ...
    featureInstruction:
      ...
      'CogxAI\'s approach (Stanford Generative Agents, 4-tier memory, dream cycles, on-chain verification) is unique. ' +
      'Don\'t trash competitors — acknowledge what they do well, then highlight what makes CogxAI different. ' +
